package Koha::Plugin::Com::OpenFifth::SAP::UploadController;
...
Handle upload/save operations for SAP reports
            ...
            if ( $status->{operations} && ref $status->{operations} eq 'ARRAY' )
            {
                ...
                for my $op ( reverse @{ $status->{operations} } ) {
                    if ( $op->{status} eq 'error' && $op->{detail} ) {
                        ...
                        if (   defined $op->{detail}->{error_raw}
                            && $op->{detail}->{error_raw} ne '' )
                        {
                            $error_detail->{error_raw} =
                              $op->{detail}->{error_raw};
                        }

                        $error_detail->{operation} = $op->{code}
                          if $op->{code};
                        last;
                    }
                }
        ...
        for my $msg ( reverse @{$messages} ) {
            if ( $msg->type eq 'error' ) {
                ...
                if ($payload) {
                    ...
                    if (   defined $payload->{error_raw}
                        && $payload->{error_raw} ne '' )
                    {
